...

{
   ...
   /* Bug 209 doesn't appear to be an issue anymore. -DE */
#if 0
 /*bug 209, Liao*/
		       ...
		       CONTEXT      context)
{
   ...
   else /* Return through a preg; the usual case */
   {
	...
	if (!WN2C_assignment_compatible_types(return_ty, ST_type(result_var))) {
	 ...
	 {
       // following code only for Compile_Upc? Deepak 
#ifdef COMPILE_UPC
	   if (Compile_Upc) {
...
WN2C_binaryop(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (WN2C_IS_INFIX_OP(WN_opcode(wn)))
      ...
      status = WN2C_funcall_op(tokens, 
			       ...
			       WN_kid1(wn), 
			       context);
   else if (isGPUKernelFunc && WN2CUDA_IS_FUNCALL_OP(WN_opcode(wn)))
      status = WN2C_funcall_op(tokens, 
			       WN_opcode(wn), 
			       WN_kid0(wn), 
			       WN_kid1(wn), 
...
WN2C_func_entry(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
  ...
  if (!ST_keep_name_w2f(WN_st(wn))) {
   ...
   // Deepak: put COMPILE_UPC check around the if statement instead of 
...
WN2C_mstore(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   // Deepak: this isn't in latest Open64, removing it for now
   /*
#ifdef COMPILE_UPC
...
WN2C_cvt(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   // Should this be under COMPILE_UPC? Deepak
//#ifdef COMPILE_UPC
